...
export type HeaderNavAccessModule = 'rankings' | 'pricing'
...
const ACCESS_DEFAULTS: Record<HeaderNavAccessModule, ModuleAccess> = {
...
export function getModuleAccessFromStatus(
  ...
  return (
    parseHeaderNavModulesFromStatus(status)[module] ?? ACCESS_DEFAULTS[module]
  )
}

export function getModuleAccess(module: HeaderNavAccessModule): ModuleAccess {
...
export function getHeaderNavEnabledFromStatus(
  status: Record<string, unknown> | null,
  module: HeaderNavBooleanModule
): boolean {
  const modules = parseHeaderNavModulesFromStatus(status)
  return modules[module] !== false
}

export async function getFreshHeaderNavEnabled(
  module: HeaderNavBooleanModule
): Promise<boolean> {
  try {
    const status = (await getStatus()) as Record<string, unknown> | null
    cacheStatus(status)
    return getHeaderNavEnabledFromStatus(status, module)
  } catch {
    return false
  }
}
