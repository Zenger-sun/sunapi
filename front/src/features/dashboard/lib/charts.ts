...
export function processChartData(
  ...
  const unknownGroupLabel = tt('Unknown Group')
  ...
  if (!data || data.length === 0) {
    return {
      spec_pie: {
        ...
        title: {
          visible: true,
          text: tt('Group Call Count Distribution'),
  ...
  >()
  const groupTotalsMap = new Map<string, { count: number }>()

  data.forEach((item) => {
    const timestamp = Number(item.created_at)
    const timeKey = formatChartTime(timestamp, timeGranularity)
    const model = item.model_name || 'Unknown'
    const group = String(item.group || '').trim() || unknownGroupLabel
    ...
    const groupExisting = groupTotalsMap.get(group) || { count: 0 }
    groupTotalsMap.set(group, {
      count: groupExisting.count + count,
    })
  })
  ...
  const sortedGroups = Array.from(groupTotalsMap.keys()).sort()
  const groupColor = {
    type: 'ordinal',
    domain: sortedGroups,
    range: getVChartDefaultColors(sortedGroups.length, themeKey),
  }
  ...
  // Pie chart: group call count distribution
  const pieValues = Array.from(groupTotalsMap.entries())
    .map(([group, stats]) => ({
      type: group,
