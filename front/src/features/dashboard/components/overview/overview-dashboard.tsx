...
export function OverviewDashboard() {
  ...
  const startSteps = useMemo<StartStep[]>(
    () => [
      ...
      {
        title: t('Add Channel'),
        description: t('Configure an upstream provider before routing traffic'),
        to: '/channels',
        icon: RadioTower,
  ...
  const quickActions = useMemo<QuickAction[]>(
    () => [
      ...
      {
        title: t('Docs'),
        description: t('Read the local endpoint and client setup guide'),
        to: '/docs',
        icon: BookOpen,
      },
    ],
  ...
  const requestExample = useMemo<RequestExample>(() => {
    ...
    const model = modelsQuery.data?.[0] ?? 'gpt-5.4-mini'
