...
export type CodexRefreshTokenImportResponse = CodexOAuthCompleteResponse
...
export async function getChannel(id: number): Promise<GetChannelResponse> {
  const res = await api.get(`/api/channel/detail/${id}`)
  return res.data
}
...
export async function deleteChannel(
  ...
  const res = await api.delete(`/api/channel/delete/${id}`, channelActionConfig())
  return res.data
}
 ...
 */
export async function resetChannelUsedQuota(id: number): Promise<{
  success: boolean
  message?: string
}> {
  const res = await api.post(
    `/api/channel/reset_used_quota/${id}`,
    null,
    channelActionConfig()
  )
  return res.data
...
/**
 * Reset all channels' used quota
 */
export async function resetAllChannelsUsedQuota(): Promise<{
  success: boolean
  message?: string
  data?: number
}> {
  const res = await api.post(
    '/api/channel/reset_used_quota',
    null,
    channelActionConfig()
  )
  return res.data
...
export async function importCodexRefreshToken(
  refreshToken: string,
  proxy?: string
): Promise<CodexRefreshTokenImportResponse> {
  const res = await api.post(
    '/api/channel/codex/oauth/refresh-token',
    { refresh_token: refreshToken, proxy: proxy || '' },
    channelActionConfig()
  )
  return res.data
}
...
/**
 * Get all available groups as names for legacy channel controls.
 */
