...
export async function handleResetChannelUsedQuota(
  ...
  try {
    ...
    if (response.success) {
      toast.success(i18next.t('Used quota reset'))
      queryClient?.invalidateQueries({ queryKey: channelsQueryKeys.lists() })
    } else {
    ...
    toast.error(
      _error instanceof Error
        ? _error.message
        : i18next.t('Failed to reset used quota')
    )
  }
}
...
/**
 * Reset used quota for all channels
 */
export async function handleResetAllChannelUsedQuota(
  queryClient?: QueryClient,
  onSuccess?: (updatedCount: number) => void
): Promise<void> {
  try {
    const response = await resetAllChannelsUsedQuota()
    if (response.success) {
      const count = response.data || 0
      toast.success(
        i18next.t('All channel used quota reset', {
          count,
        })
      )
      queryClient?.invalidateQueries({ queryKey: channelsQueryKeys.lists() })
      ...
      toast.error(
        response.message || i18next.t('Failed to reset all channel used quota')
      )
    }
  } catch (_error: unknown) {
    toast.error(
      _error instanceof Error
        ? _error.message
        : i18next.t('Failed to reset all channel used quota')
    )
  }
}
