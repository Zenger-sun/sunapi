...
function isCodexCredentialObject(value: unknown): boolean {
  if (!isJsonObjectValue(value)) return false
  const nested =
    (isJsonObjectValue(value.account) &&
    isJsonObjectValue(value.account.credentials)
      ? value.account.credentials
      : undefined) ||
    (isJsonObjectValue(value.credentials) ? value.credentials : undefined) ||
    (isJsonObjectValue(value.oauth) ? value.oauth : undefined) ||
    value

  if (!isJsonObjectValue(nested)) return false

  const accessToken =
    typeof nested.access_token === 'string' ? nested.access_token.trim() : ''
  const accountId =
    typeof nested.account_id === 'string'
      ? nested.account_id.trim()
...
function isCodexCredential(value: string | undefined): boolean {
  try {
    ...
    if (Array.isArray(parsed)) {
      return parsed.every((item) => isCodexCredentialObject(item))
    }
    return isCodexCredentialObject(parsed)
  } catch {
    ...
    return lines.every((line) => {
      try {
        return isCodexCredentialObject(JSON.parse(line))
      } catch {
        return false
      }
    })
