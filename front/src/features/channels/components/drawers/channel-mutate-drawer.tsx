...
function hasAdvancedSettingsValues(values: ChannelFormValues): boolean {
  return Boolean(values.priority || values.weight)
}
...
export function ChannelMutateDrawer({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      toast.warning(
        t(
          'Warning: Base URL should not end with /v1. SunAPI will add the protocol path automatically when needed. This may cause request failures.'
        ),
        { duration: 5000 }
      )
    }, 500)
                  ...
                  <ChannelApiAccessSection>
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <FormDescription>
                                ...
                                {'{'}
                                {t(
                                  '"default": "us-central1", "claude-haiku-4-5-20251001": "europe-west1"'
                                )}
                                {'}'}
                              </FormDescription>
                              <FormMessage />
                      ...
                      <FormField
                        ...
                        render={({ field }) => (
                          <FormItem>
                            ...
                            <FormDescription>
                              {t(
                                'Custom API base URL. For official channels, SunAPI has built-in addresses. Only fill this for third-party proxy sites or special endpoints. Do not add /v1 or trailing slash.'
                              )}
                            </FormDescription>
                            <FormMessage />
                          </FormItem>
                    ...
                    <ChannelAuthSection>
                                ...
                                <div className='flex flex-col gap-2'>
                                  <span>
                                    {isEditing ? (
                                      ...
                                      currentType === 57 ? (
                                        t(
                                          'Use account authorization below or paste Codex JSON, a JSON array, or one credential per line'
                                        )
                                      ) : (
                                        t(
                                      ...
                                      t(
                                        'Use account authorization below or paste a Codex JSON credential'
                                      )
                                    ) : (
                              ...
                              {isEditing && (
                                <div className='border-border/60 mt-4 flex flex-col gap-3 border-y border-dashed py-4'>
                                  <div className='flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between'>
                                    <div>
                                      ...
                                      <p className='text-muted-foreground text-xs'>
                                        {t('Click to reveal the saved key.')}
                                      </p>
                                    </div>
                                  ...
                                  <Input
                                    readOnly
                                    value={channelKey ?? ''}
                                    placeholder={t('Hidden, click reveal to view')}
                                    className='font-mono'
                                  />
                                </div>
                              )}
                      ...
                      {currentType === 57 && (
                        <div className='border-border/60 flex flex-col gap-3 border-y py-4'>
                          <div className='flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between'>
                            <div className='flex flex-col gap-0.5'>
                              <div className='text-sm font-semibold'>
                                {t('OpenAI Account Authorization')}
                              </div>
                              <div className='text-muted-foreground text-xs'>
                                {t(
                                  'Add accounts by manual authorization, Refresh Token, Mobile RT, or Codex JSON / AT import.'
                                )}
                              </div>
                            </div>
                          ...
                          <Alert>
                            <AlertDescription>
                              {t(
                                'Generated credentials are inserted into the key field. Save the channel to persist them. Batch mode creates one channel per credential.'
                              )}
                            </AlertDescription>
                          </Alert>
                        </div>
                      ...
                      <CodexOAuthDialog
                        ...
                        proxy={currentProxy}
                        onKeyGenerated={(key, count) => {
                          if (!isEditing && count > 1) {
                            form.setValue('multi_key_mode', 'batch', {
                              shouldDirty: true,
                            })
                          }
