...
import {
  ...
  handleFixAbilities,
  handleResetAllChannelUsedQuota,
...
export function ChannelsPrimaryButtons() {
  ...
  return (
    <>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align='end' className='w-56'>
            ...
            <DropdownMenuItem
              onSelect={(e) => {
                e.preventDefault()
                setShowResetQuotaDialog(true)
              }}
            >
              {t('Reset All Used Quota')}
              <DropdownMenuShortcut>
      ...
      <ConfirmDialog
        open={showResetQuotaDialog}
        onOpenChange={setShowResetQuotaDialog}
        title={t('Reset All Used Quota')}
        desc={t(
          'Reset the recorded used quota for all channels to 0? Future settled traffic will continue to be recorded using the current pricing ratios.'
        )}
        confirmText={t('Reset')}
        destructive
        isLoading={isResettingQuota}
        handleConfirm={async () => {
          setIsResettingQuota(true)
          try {
            await handleResetAllChannelUsedQuota(queryClient)
