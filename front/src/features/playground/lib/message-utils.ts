...
export function buildMessageContent(
  ...
  const validImages = images.filter(
    (image) => image.file_id || image.url.trim() !== ''
  )
  ...
  const parts: ContentPart[] = [
    ...
    ...validImages.map((image) => ({
      ...
      image_url: {
        url:
          image.url.trim() ||
          (image.file_id ? `playground-attachment://${image.file_id}` : ''),
        ...(image.file_id ? { file_id: image.file_id } : {}),
      },
    })),
  ]
...
export function formatMessageForAPIWithoutImages(
  message: Message
): ChatCompletionMessage {
  const currentVersion = getCurrentVersion(message)
...
/**
 * Remove inline image data before persistence while keeping attachment metadata.
 */
export function stripMessageImageData(messages: Message[]): Message[] {
    ...
    return {
      ...
      attachments: message.attachments.map((attachment) => {
        if (
          ...
          !attachment.url.toLowerCase().startsWith('data:image/')
        ) {
          return attachment
        }
        ...
        if (attachment.file_id) {
          return {
            ...attachment,
            url: `/api/playground/attachments/${attachment.file_id}`,
          }
        }
...
export function isValidMessage(message: Message): boolean {
  ...
  if (message.from === 'user' && !content.trim() && !hasAttachments) {
    return false
  }
...
export function updateAssistantMessageWithError(
  ...
  return updateLastAssistantMessage(messages, (message) => {
    ...
    const content =
      trimmed === ERROR_MESSAGES.API_REQUEST_ERROR ||
      trimmed.startsWith(`${ERROR_MESSAGES.API_REQUEST_ERROR}:`)
        ? trimmed
        : `${ERROR_MESSAGES.API_REQUEST_ERROR}: ${trimmed || ERROR_MESSAGES.API_REQUEST_ERROR}`
