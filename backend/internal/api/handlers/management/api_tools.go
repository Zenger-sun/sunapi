...
func googleOAuthClientCredentials(provider string) (string, string, error) {
	var idEnv, secretEnv string
	switch provider {
	case "gemini":
		idEnv = "GEMINI_OAUTH_CLIENT_ID"
		secretEnv = "GEMINI_OAUTH_CLIENT_SECRET"
	case "antigravity":
		idEnv = "ANTIGRAVITY_OAUTH_CLIENT_ID"
		secretEnv = "ANTIGRAVITY_OAUTH_CLIENT_SECRET"
	default:
		return "", "", fmt.Errorf("%s oauth provider is unsupported", provider)
	}
	clientID := strings.TrimSpace(os.Getenv(idEnv))
	clientSecret := strings.TrimSpace(os.Getenv(secretEnv))
	if clientID == "" || clientSecret == "" {
		return "", "", fmt.Errorf("%s oauth client credentials missing: set %s and %s", provider, idEnv, secretEnv)
	}
...
func (h *Handler) refreshGeminiOAuthAccessToken(ctx context.Context, auth *coreauth.Auth) (string, error) {
	...
	clientID, clientSecret, errCredentials := googleOAuthClientCredentials("gemini")
...
func (h *Handler) refreshAntigravityOAuthAccessToken(ctx context.Context, auth *coreauth.Auth) (string, error) {
	...
	clientID, clientSecret, errCredentials := googleOAuthClientCredentials("antigravity")
	if errCredentials != nil {
		return "", errCredentials
	}
