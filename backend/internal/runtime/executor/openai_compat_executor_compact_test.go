...
func TestOpenAICompatExecutorResponsesPassthroughPreservesReasoningHigh(t *testing.T) {
	var gotPath string
	var gotBody []byte
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.Path
		...
		gotBody = body
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{"id":"resp_1","object":"response","created_at":1,"model":"gpt-5.5","output":[],"usage":{"input_tokens":1,"output_tokens":2,"total_tokens":3}}`))
	}))
	defer server.Close()
	...
	payload := []byte(`{"model":"gpt-5.5","input":[{"role":"user","content":"hi"}],"reasoning":{"effort":"high"}}`)
	resp, err := executor.Execute(context.Background(), auth, cliproxyexecutor.Request{
		Model:   "gpt-5.5",
		Payload: payload,
	}, cliproxyexecutor.Options{
	...
	if gjson.GetBytes(gotBody, "reasoning_effort").Exists() {
		t.Fatalf("unexpected chat reasoning_effort in body: %s", string(gotBody))
	}
	if gjson.GetBytes(gotBody, "messages").Exists() {
		t.Fatalf("unexpected chat messages in body: %s", string(gotBody))
	}
...
func TestOpenAICompatExecutorResponsesStreamPassthroughPreservesReasoningHigh(t *testing.T) {
	var gotPath string
	var gotBody []byte
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		gotPath = r.URL.Path
		...
		_, _ = w.Write([]byte(`data: {"type":"response.completed","response":{"id":"resp_1","object":"response","created_at":1,"model":"gpt-5.5","output":[],"usage":{"input_tokens":1,"output_tokens":2,"total_tokens":3}}}` + "\n\n"))
	...
	payload := []byte(`{"model":"gpt-5.5","input":[{"role":"user","content":"hi"}],"reasoning":{"effort":"high"},"stream":true}`)
	...
	if gjson.GetBytes(gotBody, "stream_options").Exists() {
		t.Fatalf("unexpected chat stream_options in body: %s", string(gotBody))
	}
	if gjson.GetBytes(gotBody, "messages").Exists() {
		t.Fatalf("unexpected chat messages in body: %s", string(gotBody))
	}
