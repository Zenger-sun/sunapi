...
func geminiOAuthClientCredentials() (string, string, error) {
	...
	if clientID == "" || clientSecret == "" {
		return "", "", fmt.Errorf("gemini-cli oauth client credentials missing: set GEMINI_OAUTH_CLIENT_ID and GEMINI_OAUTH_CLIENT_SECRET")
	}
...
func prepareGeminiCLITokenSource(ctx context.Context, cfg *config.Config, auth *cliproxyauth.Auth) (oauth2.TokenSource, map[string]any, error) {
	...
	clientID, clientSecret, errCredentials := geminiOAuthClientCredentials()
