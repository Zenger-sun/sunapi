...
func (e *OpenAICompatExecutor) Execute(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (resp cliproxyexecutor.Response, err error) {
	...
	to, endpoint := openAICompatTarget(opts)
	...
	if openAICompatUsesResponsesEndpoint(opts) {
	...
	url := openAICompatEndpointURL(baseURL, endpoint)
...
func (e *OpenAICompatExecutor) executeImages(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, endpointPath string) (resp cliproxyexecutor.Response, err error) {
	...
	url := openAICompatEndpointURL(baseURL, endpointPath)
...
func (e *OpenAICompatExecutor) ExecuteStream(ctx context.Context, auth *cliproxyauth.Auth, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (_ *cliproxyexecutor.StreamResult, err error) {
	...
	if opts.Alt == "responses/compact" {
		err = statusErr{code: http.StatusBadRequest, msg: "streaming not supported for /responses/compact"}
		return nil, err
	}
	originalPayloadSource := req.Payload
	if len(opts.OriginalRequest) > 0 {
	...
	go func() {
		...
		for scanner.Scan() {
			...
			if detail, ok := helps.ParseOpenAIResponsesStreamUsage(trimmedLine); ok {
				reporter.Publish(ctx, detail)
			}
...
func openAICompatTarget(opts cliproxyexecutor.Options) (sdktranslator.Format, string) {
	if openAICompatUsesResponsesEndpoint(opts) {
		if opts.Alt == "responses/compact" {
...
func openAICompatUsesResponsesEndpoint(opts cliproxyexecutor.Options) bool {
	if opts.Alt == "responses/compact" {
		return true
	}
	...
	path := strings.ToLower(strings.TrimSpace(helps.PayloadRequestPath(opts)))
	return strings.HasSuffix(path, "/responses") || strings.Contains(path, "/responses?")
}

func openAICompatEndpointURL(baseURL, endpoint string) string {
	...
	if strings.HasSuffix(lower, "/v1") || strings.HasSuffix(lower, "/compatible-mode/v1") || strings.HasSuffix(lower, "/api/paas/v4") {
