...
func GetProviderName(modelName string) []string {
	...
	for _, provider := range fallbackProviderNames(modelName) {
		appendProvider(provider)
	}
...
func fallbackProviderNames(modelName string) []string {
	...
	if static := registry.LookupStaticModelInfo(model); static != nil {
		switch strings.ToLower(strings.TrimSpace(static.Type)) {
		case "claude":
	...
	case strings.HasPrefix(model, "gemini-"):
		return []string{"gemini", "vertex", "gemini-cli", "aistudio"}
	case strings.HasPrefix(model, "gpt-"), strings.HasPrefix(model, "o1"), strings.HasPrefix(model, "o3"), strings.HasPrefix(model, "o4"), strings.Contains(model, "codex"):
		return []string{"codex"}
