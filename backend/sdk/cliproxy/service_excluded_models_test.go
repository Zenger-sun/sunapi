...
func TestRegisterModelsForAuth_OpenAICompatibilityImageModelType(t *testing.T) {
	...
	if !internalregistryHasLevel(chatModel.Thinking.Levels, "xhigh") {
		t.Fatalf("chat model thinking levels = %v, want xhigh", chatModel.Thinking.Levels)
	}
}

func internalregistryHasLevel(levels []string, target string) bool {
