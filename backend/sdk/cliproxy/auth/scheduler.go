...
const maxScheduledAuthWeight = 1000
...
func (s *authScheduler) pickSingleWithStrategy(ctx context.Context, provider, model string, opts cliproxyexecutor.Options, tried map[string]struct{}, strategy schedulerStrategy) (*Auth, error) {
	...
	groupPredicate := sunAPIGroupPredicate(ctx)
	predicate := combineScheduledPredicates(basePredicate, groupPredicate)
	if picked := shard.pickReadyLocked(preferWebsocket, strategy, predicate); picked != nil {
		return picked, nil
	}
	if groupPredicate != nil && sunAPICrossGroupRetry(ctx) {
		if picked := shard.pickReadyLocked(preferWebsocket, strategy, basePredicate); picked != nil {
			return picked, nil
		}
		return nil, shard.unavailableErrorLocked(provider, model, basePredicate)
	}
...
func authWeight(auth *Auth) int {
	...
	if parsed > maxScheduledAuthWeight {
		return maxScheduledAuthWeight
	}
	return parsed
}

func authSunAPIGroup(auth *Auth) string {
	if auth == nil || auth.Attributes == nil {
		return ""
	}
	return strings.TrimSpace(auth.Attributes["sunapi_group"])
}

func sunAPIGroupPredicate(ctx context.Context) func(*scheduledAuth) bool {
	group := sunAPIRequestGroup(ctx)
	if group == "" {
		return nil
	}
	return func(entry *scheduledAuth) bool {
		...
		authGroup := strings.TrimSpace(entry.meta.sunAPIGroup)
		return authGroup == group
	}
}

func sunAPIRequestGroup(ctx context.Context) string {
	if ctx == nil {
		return ""
	}
	raw, ok := sunAPIAccessMetadata(ctx)
	if !ok {
		return ""
	}
	return strings.TrimSpace(accessMetadataString(raw, "api_key_group"))
}

func sunAPICrossGroupRetry(ctx context.Context) bool {
	if ctx == nil {
		return false
	}
	raw, ok := sunAPIAccessMetadata(ctx)
	...
	value := strings.TrimSpace(accessMetadataString(raw, "api_key_cross_group_retry"))
...
func sunAPIAccessMetadata(ctx context.Context) (any, bool) {
...
func accessMetadataString(raw any, key string) string {
	switch v := raw.(type) {
	case map[string]string:
		return strings.TrimSpace(v[key])
	case map[string]any:
		return contextStringValue(v[key])
	default:
		return ""
	}
}
...
func buildScheduledAuthMeta(auth *Auth) *scheduledAuthMeta {
	...
	return &scheduledAuthMeta{
		...
		sunAPIGroup:       authSunAPIGroup(auth),
...
func buildReadyView(entries []*scheduledAuth) readyView {
	view := readyView{flat: weightedReadyEntries(entries)}
	if len(entries) == 0 {
		return view
	}
...
func weightedReadyEntries(entries []*scheduledAuth) []*scheduledAuth {
	if len(entries) == 0 {
		return nil
	}
	total := 0
	...
	out := make([]*scheduledAuth, 0, total)
	for _, entry := range entries {
		weight := scheduledAuthWeight(entry)
		for index := 0; index < weight; index++ {
