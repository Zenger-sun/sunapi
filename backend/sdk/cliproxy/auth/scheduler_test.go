...
type schedulerGinContext map[string]any

func (c schedulerGinContext) Get(key string) (any, bool) {
	value, ok := c[key]
	return value, ok
}

func sunAPISchedulerContext(group string, crossGroupRetry bool) context.Context {
	metadata := map[string]string{"api_key_group": group}
	if crossGroupRetry {
		metadata["api_key_cross_group_retry"] = "true"
	}
	return context.WithValue(context.Background(), "gin", schedulerGinContext{"accessMetadata": metadata})
}
...
func TestSchedulerPick_RoundRobinHonorsAuthWeight(t *testing.T) {
	t.Parallel()

	scheduler := newSchedulerForTest(
		&RoundRobinSelector{},
		&Auth{ID: "heavy", Provider: "openai-compatible", Attributes: map[string]string{"weight": "2"}},
		&Auth{ID: "light", Provider: "openai-compatible", Attributes: map[string]string{"weight": "1"}},
	)

	want := []string{"heavy", "heavy", "light", "heavy", "heavy", "light"}
	for index, wantID := range want {
		got, errPick := scheduler.pickSingle(context.Background(), "openai-compatible", "", cliproxyexecutor.Options{}, nil)
		if errPick != nil {
			t.Fatalf("pickSingle() #%d error = %v", index, errPick)
		}
		if got == nil {
...
func TestSchedulerPick_SunAPIGroupFiltersByAPIKeyGroup(t *testing.T) {
	t.Parallel()

	scheduler := newSchedulerForTest(
		&RoundRobinSelector{},
		&Auth{ID: "default", Provider: "sunapi-1-default", Attributes: map[string]string{"sunapi_group": "default"}},
		&Auth{ID: "vip", Provider: "sunapi-2-vip", Attributes: map[string]string{"sunapi_group": "vip"}},
	)

	got, provider, errPick := scheduler.pickMixed(sunAPISchedulerContext("vip", false), []string{"sunapi-1-default", "sunapi-2-vip"}, "", cliproxyexecutor.Options{}, nil)
	if errPick != nil {
	...
	if got.ID != "vip" || provider != "sunapi-2-vip" {
		t.Fatalf("pickMixed() = auth %q provider %q, want vip/sunapi-2-vip", got.ID, provider)
	}
}

func TestSchedulerPick_SunAPIGroupRejectsUntaggedAuth(t *testing.T) {
	t.Parallel()

	scheduler := newSchedulerForTest(
		&RoundRobinSelector{},
		&Auth{ID: "untagged", Provider: "sunapi-1-default"},
		&Auth{ID: "vip", Provider: "sunapi-2-vip", Attributes: map[string]string{"sunapi_group": "vip"}},
	)
...
func TestSchedulerPick_SunAPICrossGroupRetryFallsBackAfterGroupTried(t *testing.T) {
	t.Parallel()
	...
	tried := map[string]struct{}{"vip": {}}

	got, provider, errPick := scheduler.pickMixed(sunAPISchedulerContext("vip", true), []string{"sunapi-1-default", "sunapi-2-vip"}, "", cliproxyexecutor.Options{}, tried)
	if errPick != nil {
		t.Fatalf("pickMixed() error = %v", errPick)
	}
	if got == nil {
	...
	if got.ID != "default" || provider != "sunapi-1-default" {
		t.Fatalf("pickMixed() = auth %q provider %q, want default/sunapi-1-default", got.ID, provider)
	}
}
