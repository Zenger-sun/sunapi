...
type delayedFirstChunkStreamExecutor struct {
	delay time.Duration
}
...
func (e *delayedFirstChunkStreamExecutor) Identifier() string { return "codex" }

func (e *delayedFirstChunkStreamExecutor) Execute(context.Context, *coreauth.Auth, coreexecutor.Request, coreexecutor.Options) (coreexecutor.Response, error) {
	return coreexecutor.Response{}, &coreauth.Error{Code: "not_implemented", Message: "Execute not implemented"}
}

func (e *delayedFirstChunkStreamExecutor) ExecuteStream(context.Context, *coreauth.Auth, coreexecutor.Request, coreexecutor.Options) (*coreexecutor.StreamResult, error) {
	ch := make(chan coreexecutor.StreamChunk, 1)
	go func() {
		time.Sleep(e.delay)
		ch <- coreexecutor.StreamChunk{Payload: []byte("slow-ok")}
		close(ch)
	}()
	return &coreexecutor.StreamResult{Chunks: ch}, nil
}

func (e *delayedFirstChunkStreamExecutor) Refresh(ctx context.Context, auth *coreauth.Auth) (*coreauth.Auth, error) {
	return auth, nil
}

func (e *delayedFirstChunkStreamExecutor) CountTokens(context.Context, *coreauth.Auth, coreexecutor.Request, coreexecutor.Options) (coreexecutor.Response, error) {
	return coreexecutor.Response{}, &coreauth.Error{Code: "not_implemented", Message: "CountTokens not implemented"}
}

func (e *delayedFirstChunkStreamExecutor) HttpRequest(ctx context.Context, auth *coreauth.Auth, req *http.Request) (*http.Response, error) {
	return nil, &coreauth.Error{
		Code:       "not_implemented",
		Message:    "HttpRequest not implemented",
		HTTPStatus: http.StatusNotImplemented,
...
func TestExecuteStreamWithAuthManager_ReturnsBeforeSlowFirstChunk(t *testing.T) {
	executor := &delayedFirstChunkStreamExecutor{delay: 600 * time.Millisecond}
	manager := coreauth.NewManager(nil, nil, nil)
	manager.RegisterExecutor(executor)

	auth := &coreauth.Auth{
		ID:       "slow-first-chunk-auth",
		Provider: "codex",
		Status:   coreauth.StatusActive,
		Metadata: map[string]any{"email": "slow@example.com"},
	}
	if _, err := manager.Register(context.Background(), auth); err != nil {
		t.Fatalf("manager.Register(auth): %v", err)
	}

	registry.GetGlobalRegistry().RegisterClient(auth.ID, auth.Provider, []*registry.ModelInfo{{ID: "slow-first-chunk-model"}})
	t.Cleanup(func() {
	...
	start := time.Now()
	dataChan, _, errChan := handler.ExecuteStreamWithAuthManager(context.Background(), "openai", "slow-first-chunk-model", []byte(`{"model":"slow-first-chunk-model"}`), "")
	elapsed := time.Since(start)
	if elapsed >= 500*time.Millisecond {
		t.Fatalf("ExecuteStreamWithAuthManager waited %s for first chunk", elapsed)
	}
	...
	if string(got) != "slow-ok" {
		t.Fatalf("stream payload = %q, want slow-ok", got)
	}
}
