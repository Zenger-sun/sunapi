...
func (h *OpenAIResponsesAPIHandler) handleStreamingResponse(c *gin.Context, rawJSON []byte) {
	...
	writeTerminalStreamError := func(errMsg *interfaces.ErrorMessage) {
		if errMsg == nil {
			return
		}
	...
	firstByteKeepAlive := handlers.StreamingKeepAliveInterval(h.Cfg)
	if firstByteKeepAlive <= 0 {
		firstByteKeepAlive = 2 * time.Second
	}
	keepAlive := time.NewTicker(firstByteKeepAlive)
	...
	for {
		...
		case errMsg, ok := <-errChan:
			...
			// If the stream has not been committed yet, preserve the normal JSON error response.
			if sseStarted {
				writeTerminalStreamError(errMsg)
				flusher.Flush()
			} else {
				h.WriteErrorResponse(c, errMsg)
			}
