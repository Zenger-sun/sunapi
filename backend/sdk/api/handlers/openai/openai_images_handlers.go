...
func isSupportedImagesModel(model string) bool {
	...
	return isXAIImagesModel(model) || isOpenAICompatImagesModel(model) || isConfiguredOpenAICompatImagesToolModel(model)
}
...
func isConfiguredOpenAICompatImagesToolModel(model string) bool {
	if !isOpenAICompatImagesModel(model) && !imagesModelLooksLikeImageTool(model) {
		return false
	}
	return len(openAICompatImageProviderKeys(model)) > 0
}

func openAICompatImageProviderKeys(imageModel string) []string {
	providers := registry.GetGlobalRegistry().GetModelProviders(imageModel)
	if _, baseModel := imagesModelParts(imageModel); baseModel != "" && !strings.EqualFold(strings.TrimSpace(imageModel), baseModel) {
		providers = append(providers, registry.GetGlobalRegistry().GetModelProviders(baseModel)...)
	}
	result := make([]string, 0, len(providers))
	seen := make(map[string]struct{}, len(providers))
	for _, provider := range providers {
		provider = strings.ToLower(strings.TrimSpace(provider))
		if provider != "" && provider != "codex" && provider != "xai" {
...
func imagesModelLooksLikeImageTool(model string) bool {
	baseModel := imagesModelBase(model)
	return strings.Contains(baseModel, "image")
}
...
func (h *OpenAIAPIHandler) openAICompatImagesResponsesMainModel(c *gin.Context, imageModel string) string {
	models := h.openAICompatImagesResponsesMainModels(c, imageModel)
	if len(models) == 0 {
		return ""
	}
	return models[0]
...
func (h *OpenAIAPIHandler) openAICompatImagesResponsesMainModels(c *gin.Context, imageModel string) []string {
	if h == nil || h.BaseAPIHandler == nil || h.AuthManager == nil {
		return nil
	}
	group := imagesRequestGroup(c)
	result := make([]string, 0)
	seen := make(map[string]struct{})
	for _, provider := range openAICompatImageProviderKeys(imageModel) {
		for _, model := range h.openAICompatResponsesMainModelsForProvider(provider, group, imageModel) {
			...
			if len(result) >= imagesMainModelMaxAttempts {
				return result
			}
		}
	}
...
func (h *OpenAIAPIHandler) firstOpenAICompatResponsesMainModel(provider string, group string, imageModel string) string {
	models := h.openAICompatResponsesMainModelsForProvider(provider, group, imageModel)
	if len(models) == 0 {
		return ""
	}
	return models[0]
...
func (h *OpenAIAPIHandler) openAICompatResponsesMainModelsForProvider(provider string, group string, imageModel string) []string {
	var candidates []imagesMainModelCandidate
	modelRegistry := registry.GetGlobalRegistry()
	for _, auth := range h.AuthManager.List() {
		if !imagesMainModelAuthUsable(auth) {
			continue
		}
		if strings.ToLower(strings.TrimSpace(auth.Provider)) != provider {
			continue
		}
		if group != "" && !strings.EqualFold(strings.TrimSpace(auth.Attributes["sunapi_group"]), group) {
			continue
		}
		models := modelRegistry.GetModelsForClient(auth.ID)
		if !modelsContainImageModel(models, imageModel) {
			continue
		}
		for _, model := range models {
			if !isOpenAICompatGPTMainModelCandidate(model) {
				continue
			}
			candidates = append(candidates, imagesMainModelCandidate{
				id:         strings.TrimSpace(model.ID),
				version:    imagesGPTVersionScore(model.ID),
				preference: imagesGPTPreferenceScore(model.ID),
...
func imagesMainModelAuthUsable(auth *coreauth.Auth) bool {
...
func imagesRequestGroup(c *gin.Context) string {
	if c == nil {
		return ""
	}
	...
	return imagesAccessMetadataString(raw, "api_key_group")
}

func injectImagesRequestGroup(c *gin.Context, payload []byte) []byte {
	group := imagesRequestGroup(c)
	if group == "" || strings.EqualFold(group, "default") || len(bytes.TrimSpace(payload)) == 0 || !json.Valid(payload) {
		return payload
	}
	updated, err := sjson.SetBytes(payload, "group", group)
	if err != nil {
		return payload
	}
	return updated
...
func imagesAccessMetadataString(raw any, key string) string {
	switch v := raw.(type) {
...
func modelsContainImageModel(models []*registry.ModelInfo, imageModel string) bool {
	for _, model := range models {
		if model == nil {
			continue
		}
		if sameImagesModelID(model.ID, imageModel) && (model.Type == registry.OpenAIImageModelType || imagesModelLooksLikeImageTool(model.ID)) {
			return true
		}
	}
	return false
...
func sameImagesModelID(a string, b string) bool {
	a = strings.TrimSpace(a)
	b = strings.TrimSpace(b)
	return strings.EqualFold(a, b) || imagesModelBase(a) == imagesModelBase(b)
}

func isOpenAICompatGPTMainModelCandidate(model *registry.ModelInfo) bool {
	...
	base := imagesModelBase(id)
	if !strings.HasPrefix(base, "gpt-") && !strings.HasPrefix(base, "chatgpt-") {
		return false
	}
	for _, blocked := range []string{"image", "audio", "embedding", "moderation", "rerank", "codex", "auto-review", "review"} {
		if strings.Contains(base, blocked) {
			return false
		}
	}
	return imagesGPTVersionScore(id) > 0
}

func imagesGPTVersionScore(id string) int {
	base := imagesModelBase(id)
	idx := strings.Index(base, "gpt-")
	if idx < 0 {
		return 0
	}
	rest := base[idx+len("gpt-"):]
	parts := [3]int{}
	partIdx := 0
	num := -1
	for i := 0; i < len(rest) && partIdx < len(parts); i++ {
		ch := rest[i]
		if ch >= '0' && ch <= '9' {
			if num < 0 {
				num = 0
	...
	if num >= 0 && partIdx < len(parts) {
		parts[partIdx] = num
	}
	return parts[0]*1000000 + parts[1]*1000 + parts[2]
}

func imagesGPTPreferenceScore(id string) int {
	base := imagesModelBase(id)
	score := 100
	for _, term := range []string{"mini", "lite", "small"} {
		if strings.Contains(base, term) {
			score -= 20
		}
	}
	if strings.Contains(base, "nano") {
		score -= 30
	}
	if strings.Contains(base, "preview") || strings.Contains(base, "beta") {
		score -= 5
	}
...
func xaiImagesAspectRatioFromSize(size string, fallback string) string {
	...
	switch size {
	case "1024x1024", "2048x2048", "4096x4096", "1:1":
		return "1:1"
	case "1792x1024", "2048x1152", "4096x2304", "16:9":
		return "16:9"
	case "1024x1792", "1152x2048", "2304x4096", "9:16":
		return "9:16"
	case "1344x1024", "2048x1536", "4096x3072", "4:3":
		return "4:3"
	case "1024x1344", "1536x2048", "3072x4096", "3:4":
		return "3:4"
	case "1536x1024", "2048x1365", "4096x2731", "3:2":
		return "3:2"
	case "1024x1536", "1365x2048", "2731x4096", "2:3":
		return "2:3"
	default:
		return fallback
	}
...
func xaiImagesResolution(raw string, size string, fallback string) string {
	...
	if strings.Contains(strings.ToLower(strings.TrimSpace(size)), "4096") {
		return "4k"
	}
	return fallback
}
...
func buildOpenAICompatImagesJSONRequest(rawJSON []byte, imageModel string, stream bool) []byte {
	payload := normalizeImagesJSONOptions(rawJSON)
...
func normalizeImagesJSONOptions(rawJSON []byte) []byte {
	payload := rawJSON
	if !json.Valid(payload) {
		return payload
	}

	resolution := normalizeImagesResolution(gjson.GetBytes(payload, "resolution").String())
	aspectRatio := normalizeImagesAspectRatio(gjson.GetBytes(payload, "aspect_ratio").String())
	size := normalizeImagesSize(gjson.GetBytes(payload, "size").String())
	if resolution == "auto" {
		payload, _ = sjson.DeleteBytes(payload, "resolution")
		resolution = ""
	}
	if aspectRatio == "auto" {
		payload, _ = sjson.DeleteBytes(payload, "aspect_ratio")
		aspectRatio = ""
	}
	if size == "auto" {
		payload, _ = sjson.DeleteBytes(payload, "size")
		size = ""
	}
	if size == "" {
		size = imageSizeForOptions(resolution, aspectRatio)
		if size != "" {
			payload, _ = sjson.SetBytes(payload, "size", size)
		}
	}
	if aspectRatio != "" {
		payload = enforceImageAspectRatioInPrompt(payload, aspectRatio)
	}
	return payload
}

func normalizeImagesResolution(raw string) string {
	switch strings.ToLower(strings.TrimSpace(raw)) {
	case "auto":
		return "auto"
	case "1k", "2k", "4k":
...
func normalizeImagesAspectRatio(raw string) string {
	if strings.EqualFold(strings.TrimSpace(raw), "auto") {
		return "auto"
	}
	return xaiImagesAspectRatio(raw, "")
}

func normalizeImagesSize(raw string) string {
	...
	if xaiImagesAspectRatioFromSize(size, "") != "" {
		return size
	}
	return strings.TrimSpace(raw)
}

func imageSizeForOptions(resolution string, aspectRatio string) string {
	resolution = normalizeImagesResolution(resolution)
	aspectRatio = normalizeImagesAspectRatio(aspectRatio)
...
func enforceImageAspectRatioInPrompt(payload []byte, aspectRatio string) []byte {
	prompt := strings.TrimSpace(gjson.GetBytes(payload, "prompt").String())
	if prompt == "" || promptMentionsImageAspectRatio(prompt) {
		return payload
	}
	prompt = prompt + "\n\nCanvas requirement: generate the final image with a " + aspectRatio + " aspect ratio. Do not change the canvas ratio."
	updated, err := sjson.SetBytes(payload, "prompt", prompt)
	if err != nil {
		return payload
	}
	return updated
...
func promptMentionsImageAspectRatio(prompt string) bool {
	...
	ratioTerms := []string{
		...
		"aspect ratio", "landscape orientation", "portrait orientation", "square image",
		"比例", "宽高比", "横版", "横屏", "竖版", "竖屏", "方图", "正方形",
	}
	for _, term := range ratioTerms {
...
func (h *OpenAIAPIHandler) ImagesGenerations(c *gin.Context) {
	...
	rawJSON = normalizeImagesJSONOptions(rawJSON)
	...
	if isConfiguredOpenAICompatImagesToolModel(imageModel) {
		tool := buildImagesToolFromJSON(rawJSON, imageModel, "generate", []string{"size", "quality", "background", "output_format", "moderation"}, []string{"output_compression", "partial_images"})
		if mainModels := h.openAICompatImagesResponsesMainModels(c, imageModel); len(mainModels) > 0 {
			if stream {
				responsesReq := buildImagesResponsesRequestWithMainModel(prompt, nil, tool, mainModels[0])
				h.streamImagesFromResponses(c, responsesReq, responseFormat, "image_generation")
				return
			}
			h.collectImagesFromResponsesWithMainModels(c, prompt, nil, tool, responseFormat, mainModels)
			return
		}
	}
...
func (h *OpenAIAPIHandler) imagesEditsFromMultipart(c *gin.Context) {
	...
	if isConfiguredOpenAICompatImagesToolModel(imageModel) {
		tool := buildImagesEditToolFromForm(form, imageModel)
		if mainModels := h.openAICompatImagesResponsesMainModels(c, imageModel); len(mainModels) > 0 {
			if stream {
				responsesReq := buildImagesResponsesRequestWithMainModel(prompt, images, tool, mainModels[0])
				h.streamImagesFromResponses(c, responsesReq, responseFormat, "image_edit")
				return
			}
			h.collectImagesFromResponsesWithMainModels(c, prompt, images, tool, responseFormat, mainModels)
			return
		}
	}
	if isDefaultImagesToolModel(imageModel) {
...
func (h *OpenAIAPIHandler) imagesEditsFromJSON(c *gin.Context) {
	...
	if isConfiguredOpenAICompatImagesToolModel(imageModel) {
		images := collectImageURLsFromJSON(rawJSON)
		if len(images) == 0 {
			c.JSON(http.StatusBadRequest, handlers.ErrorResponse{
				Error: handlers.ErrorDetail{
		...
		tool := buildImagesToolFromJSON(rawJSON, imageModel, "edit", []string{"size", "quality", "background", "output_format", "input_fidelity", "moderation"}, []string{"output_compression", "partial_images"})
...
func buildImagesResponsesRequest(prompt string, images []string, toolJSON []byte) []byte {
	return buildImagesResponsesRequestWithMainModel(prompt, images, toolJSON, "")
}

func buildImagesResponsesRequestWithMainModel(prompt string, images []string, toolJSON []byte, mainModel string) []byte {
...
func buildImagesToolFromJSON(rawJSON []byte, imageModel string, action string, stringFields []string, numberFields []string) []byte {
...
func buildImagesEditToolFromForm(form *multipart.Form, imageModel string) []byte {
...
func collectImageURLsFromJSON(rawJSON []byte) []string {
...
func (h *OpenAIAPIHandler) collectRoutedImages(c *gin.Context, imageReq []byte, imageModel string) {
	...
	if !imagesAPIResponseHasImage(resp) {
...
func imagesAPIResponseHasImage(payload []byte) bool {
	data := gjson.GetBytes(payload, "data")
	if !data.IsArray() {
		return false
	}
...
func (h *OpenAIAPIHandler) collectImagesFromResponses(c *gin.Context, responsesReq []byte, responseFormat string) {
	...
	responsesReq = injectImagesRequestGroup(c, responsesReq)
	mainModel := strings.TrimSpace(gjson.GetBytes(responsesReq, "model").String())
	if mainModel == "" {
		mainModel = defaultImagesMainModel
	}
	out, upstreamHeaders, errMsg := h.collectImagesFromResponsesOnce(cliCtx, responsesReq, responseFormat, mainModel)
	if errMsg != nil {
		h.WriteErrorResponse(c, errMsg)
		if errMsg.Error != nil {
			cliCancel(errMsg.Error)
...
func (h *OpenAIAPIHandler) collectImagesFromResponsesWithMainModels(c *gin.Context, prompt string, images []string, toolJSON []byte, responseFormat string, mainModels []string) {
	c.Header("Content-Type", "application/json")
	...
	for index, mainModel := range mainModels {
		mainModel = strings.TrimSpace(mainModel)
		if mainModel == "" {
			continue
		}
		responsesReq := buildImagesResponsesRequestWithMainModel(prompt, images, toolJSON, mainModel)
		responsesReq = injectImagesRequestGroup(c, responsesReq)
		out, upstreamHeaders, errMsg := h.collectImagesFromResponsesOnce(cliCtx, responsesReq, responseFormat, mainModel)
		if errMsg == nil {
			handlers.WriteUpstreamHeaders(c.Writer.Header(), upstreamHeaders)
		...
		lastErr = errMsg
		if index+1 < len(mainModels) {
			log.Warnf("sunapi image generation main model failed, retrying with next candidate: model=%s error=%v", mainModel, errMsg.Error)
		}
	}
	if lastErr == nil {
		lastErr = &interfaces.ErrorMessage{StatusCode: http.StatusBadGateway, Error: fmt.Errorf("no compatible GPT main model available for image generation")}
	}
	h.WriteErrorResponse(c, lastErr)
	if lastErr.Error != nil {
		cliCancel(lastErr.Error)
	}
}

func (h *OpenAIAPIHandler) collectImagesFromResponsesOnce(ctx context.Context, responsesReq []byte, responseFormat string, mainModel string) ([]byte, http.Header, *interfaces.ErrorMessage) {
	dataChan, upstreamHeaders, errChan := h.ExecuteStreamWithAuthManager(ctx, "openai-response", mainModel, responsesReq, "")
	out, errMsg := collectImagesFromResponsesStream(ctx, dataChan, errChan, responseFormat)
	return out, upstreamHeaders, errMsg
}
...
func collectImagesFromResponsesStream(ctx context.Context, data <-chan []byte, errs <-chan *interfaces.ErrorMessage, responseFormat string) ([]byte, *interfaces.ErrorMessage) {
	...
	processFrame := func(frame []byte) ([]byte, bool, *interfaces.ErrorMessage) {
		for _, line := range bytes.Split(frame, []byte("\n")) {
			...
			results, createdAt, usageRaw, firstMeta, err := extractImagesFromResponsesCompleted(payload, outputItemsByIndex, outputItemsFallback)
			if err != nil {
...
func collectImagesOutputItemDone(payload []byte, itemsByIndex map[int64][]byte, fallback *[][]byte) {
	...
	if outputIndex := gjson.GetBytes(payload, "output_index"); outputIndex.Exists() {
		itemsByIndex[outputIndex.Int()] = append([]byte(nil), item.Raw...)
		return
	}
	if fallback != nil {
		*fallback = append(*fallback, append([]byte(nil), item.Raw...))
	}
}

func extractImagesFromResponsesCompleted(payload []byte, itemsByIndex map[int64][]byte, fallback [][]byte) (results []imageCallResult, createdAt int64, usageRaw []byte, firstMeta imageCallResult, err error) {
	...
	if output := gjson.GetBytes(payload, "response.output"); output.Exists() && output.IsArray() && len(output.Array()) > 0 {
		...
		for _, index := range indexes {
			appendItem(gjson.ParseBytes(itemsByIndex[index]))
		}
...
func (h *OpenAIAPIHandler) forwardImagesStream(ctx context.Context, c *gin.Context, flusher http.Flusher, cancel func(error), data <-chan []byte, errs <-chan *interfaces.ErrorMessage, firstChunk []byte, responseFormat string, streamPrefix string, writeEvent func(string, []byte)) {
	...
	processFrame := func(frame []byte) (done bool) {
		for _, line := range bytes.Split(frame, []byte("\n")) {
			...
			case "response.completed":
				results, _, usageRaw, _, err := extractImagesFromResponsesCompleted(payload, outputItemsByIndex, outputItemsFallback)
				if err != nil {
