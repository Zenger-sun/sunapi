...
func TestConfiguredOpenAICompatImageToolUsesLatestGPTInSameGroup(t *testing.T) {
	modelRegistry := registry.GetGlobalRegistry()
	image2ClientID := "test-openai-compat-image-tool-responses-main-image2"
	defaultClientID := "test-openai-compat-image-tool-responses-main-default"
	modelRegistry.RegisterClient(image2ClientID, "sunapi-image2", []*registry.ModelInfo{
		{ID: "codex-auto-review", Object: "model", OwnedBy: "compat", Type: "openai-compatibility"},
		{ID: "gpt-5.4-mini", Object: "model", OwnedBy: "compat", Type: "openai-compatibility"},
		{ID: "gpt-5.5", Object: "model", OwnedBy: "compat", Type: "openai-compatibility"},
		{ID: "gpt-image-1.5", Object: "model", OwnedBy: "compat", Type: registry.OpenAIImageModelType},
	})
	modelRegistry.RegisterClient(defaultClientID, "sunapi-image2", []*registry.ModelInfo{
		{ID: "gpt-5.9", Object: "model", OwnedBy: "compat", Type: "openai-compatibility"},
		{ID: "gpt-image-1.5", Object: "model", OwnedBy: "compat", Type: registry.OpenAIImageModelType},
	})
	t.Cleanup(func() {
		modelRegistry.UnregisterClient(image2ClientID)
		modelRegistry.UnregisterClient(defaultClientID)
	})

	manager := coreauth.NewManager(nil, nil, nil)
	for _, auth := range []*coreauth.Auth{
		{ID: image2ClientID, Provider: "sunapi-image2", Status: coreauth.StatusActive, Attributes: map[string]string{"sunapi_group": "image2"}},
		{ID: defaultClientID, Provider: "sunapi-image2", Status: coreauth.StatusActive, Attributes: map[string]string{"sunapi_group": "default"}},
	} {
	...
	c.Set("accessMetadata", map[string]string{"api_key_group": "image2"})

	if !isConfiguredOpenAICompatImagesToolModel("gpt-image-1.5") {
		t.Fatal("expected configured openai-compatibility image model to be detected")
	}
	if got := handler.openAICompatImagesResponsesMainModel(c, "gpt-image-1.5"); got != "gpt-5.5" {
		t.Fatalf("main model = %q, want gpt-5.5", got)
	}
	if got := handler.openAICompatImagesResponsesMainModels(c, "gpt-image-1.5"); len(got) != 2 || got[0] != "gpt-5.5" || got[1] != "gpt-5.4-mini" {
		t.Fatalf("main model candidates = %#v, want [gpt-5.5 gpt-5.4-mini]", got)
	}

	tool := buildImagesToolFromJSON([]byte(`{"model":"gpt-image-1.5","size":"1024x1024","quality":"auto"}`), "gpt-image-1.5", "generate", []string{"size", "quality"}, nil)
	req := buildImagesResponsesRequestWithMainModel("draw", nil, tool, handler.openAICompatImagesResponsesMainModel(c, "gpt-image-1.5"))
	if got := gjson.GetBytes(req, "model").String(); got != "gpt-5.5" {
		t.Fatalf("responses model = %q, want gpt-5.5; body=%s", got, string(req))
	}
	if got := gjson.GetBytes(req, "tools.0.model").String(); got != "gpt-image-1.5" {
		t.Fatalf("tool model = %q, want gpt-image-1.5; body=%s", got, string(req))
	}
	if got := gjson.GetBytes(req, "tools.0.action").String(); got != "generate" {
		t.Fatalf("tool action = %q, want generate; body=%s", got, string(req))
	}
}

func TestConfiguredOpenAICompatImageNameUsesResponsesEvenWhenModelTypeIsGeneric(t *testing.T) {
	modelRegistry := registry.GetGlobalRegistry()
	clientID := "test-openai-compat-generic-image-tool-responses-main"
	modelRegistry.RegisterClient(clientID, "sunapi-generic-image", []*registry.ModelInfo{
		{ID: "gpt-5.5", Object: "model", OwnedBy: "compat", Type: "openai-compatibility"},
		{ID: "gpt-image-2", Object: "model", OwnedBy: "compat", Type: "openai-compatibility"},
	})
	t.Cleanup(func() {
		modelRegistry.UnregisterClient(clientID)
	})
	...
	if _, err := manager.Register(coreauth.WithSkipPersist(context.Background()), &coreauth.Auth{
		ID:         clientID,
		Provider:   "sunapi-generic-image",
		Status:     coreauth.StatusActive,
		Attributes: map[string]string{"sunapi_group": "image2"},
	}); err != nil {
		t.Fatalf("register auth: %v", err)
	}
	handler := NewOpenAIAPIHandler(handlers.NewBaseAPIHandlers(&sdkconfig.SDKConfig{}, manager))
	...
	if !isSupportedImagesModel("gpt-image-2") {
		t.Fatal("expected image-like configured model to be supported")
	}
	if !isConfiguredOpenAICompatImagesToolModel("gpt-image-2") {
		t.Fatal("expected image-like configured model to use responses image tool path")
	}
	if got := handler.openAICompatImagesResponsesMainModel(c, "gpt-image-2"); got != "gpt-5.5" {
		t.Fatalf("main model = %q, want gpt-5.5", got)
	}
}

func TestInjectImagesRequestGroupForNonDefaultPlaygroundGroup(t *testing.T) {
	gin.SetMode(gin.TestMode)
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Set("accessMetadata", map[string]string{"api_key_group": "image2"})

	req := injectImagesRequestGroup(c, []byte(`{"model":"gpt-5.5","input":"draw"}`))
	if got := gjson.GetBytes(req, "group").String(); got != "image2" {
		t.Fatalf("group = %q, want image2; body=%s", got, string(req))
	}
}

func TestInjectImagesRequestGroupSkipsDefaultGroup(t *testing.T) {
	gin.SetMode(gin.TestMode)
	c, _ := gin.CreateTestContext(httptest.NewRecorder())
	c.Set("accessMetadata", map[string]string{"api_key_group": "default"})

	req := injectImagesRequestGroup(c, []byte(`{"model":"gpt-5.5","input":"draw"}`))
	if gjson.GetBytes(req, "group").Exists() {
		t.Fatalf("default group should not be injected: %s", string(req))
	}
}
...
func TestNormalizeImagesJSONOptionsAutoLeavesSizeUnset(t *testing.T) {
	req := normalizeImagesJSONOptions([]byte(`{"prompt":"draw a poster","resolution":"auto","aspect_ratio":"auto","size":"auto"}`))

	if gjson.GetBytes(req, "resolution").Exists() {
		t.Fatalf("resolution auto should be removed: %s", string(req))
	}
	if gjson.GetBytes(req, "aspect_ratio").Exists() {
		t.Fatalf("aspect_ratio auto should be removed: %s", string(req))
	}
	if gjson.GetBytes(req, "size").Exists() {
		t.Fatalf("size auto should be removed: %s", string(req))
	}
	if got := gjson.GetBytes(req, "prompt").String(); got != "draw a poster" {
		t.Fatalf("prompt = %q, want unchanged", got)
	}
}

func TestNormalizeImagesJSONOptionsInfersSizeAndPromptConstraint(t *testing.T) {
	req := normalizeImagesJSONOptions([]byte(`{"prompt":"draw a poster","resolution":"auto","aspect_ratio":"2:3"}`))

	if got := gjson.GetBytes(req, "size").String(); got != "1024x1536" {
		t.Fatalf("size = %q, want 1024x1536; body=%s", got, string(req))
	}
	prompt := gjson.GetBytes(req, "prompt").String()
	if !strings.Contains(prompt, "2:3 aspect ratio") {
		t.Fatalf("prompt missing aspect ratio constraint: %q", prompt)
	}
}

func TestNormalizeImagesJSONOptionsKeepsExplicitPromptRatio(t *testing.T) {
	req := normalizeImagesJSONOptions([]byte(`{"prompt":"draw a 16:9 poster","resolution":"1k","aspect_ratio":"16:9"}`))

	if got := gjson.GetBytes(req, "size").String(); got != "1792x1024" {
		t.Fatalf("size = %q, want 1792x1024; body=%s", got, string(req))
	}
	if got := strings.Count(gjson.GetBytes(req, "prompt").String(), "Canvas requirement"); got != 0 {
		t.Fatalf("prompt should not add duplicate ratio constraint: %q", gjson.GetBytes(req, "prompt").String())
	}
}
...
func TestImagesAPIResponseHasImage(t *testing.T) {
	tests := []struct {
		name    string
		payload string
		want    bool
	}{
		{name: "url", payload: `{"data":[{"url":"https://example.com/a.png"}]}`, want: true},
		{name: "base64", payload: `{"data":[{"b64_json":"AA=="}]}`, want: true},
		{name: "empty data", payload: `{"created":1782112525,"data":[]}`, want: false},
		{name: "metadata only", payload: `{"data":[{"revised_prompt":"draw a square"}]}`, want: false},
		{name: "missing data", payload: `{}`, want: false},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := imagesAPIResponseHasImage([]byte(tt.payload)); got != tt.want {
				t.Fatalf("imagesAPIResponseHasImage() = %v, want %v", got, tt.want)
			}
		})
	}
}
...
func TestCollectImagesFromResponsesStreamUsesOutputItemDoneFallback(t *testing.T) {
	...
	close(errs)
	data <- []byte(`data: {"type":"response.output_item.done","output_index":0,"item":{"type":"image_generation_call","output_format":"png","result":"QUJD"}}` + "\n\n")
	data <- []byte(`data: {"type":"response.completed","response":{"created_at":123,"output":[]}}` + "\n\n")
	close(data)
	...
	if got := gjson.GetBytes(out, "data.0.url").String(); got != "data:image/png;base64,QUJD" {
		t.Fatalf("data.0.url = %q; body=%s", got, string(out))
	}
}
